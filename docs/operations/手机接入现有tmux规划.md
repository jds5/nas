...
2026-09-16：已开始实现独立子项目 [apps/android](../../apps/android/README.md)，分为 Android 界面 `app` 与纯 Kotlin SSH/tmux 核心 `core`。首版使用 SSH exec + `capture-pane` 轮询及 bracketed paste，替代上文概念方案中的 PTY 附着，因此不改变电脑终端尺寸。只对前台命令为 `codex` 的窗格开放输入；其他窗格只读。

已实现固定主机指纹、公钥认证、Keystore 加密存储、窗格选择、输出查看、中文草稿、特殊键、后台断开与不确定发送保护。独立临时 tmux server 的测试覆盖中文多行、输入不作为 shell 命令执行、目标身份变化拒绝写入，以及保持同一进程。Material 3 使用核对后的最新稳定版本，流畅度仍待真机测量。

尚未进行手机登录与当前对话验收，未更改生产 SSH/VPN/tmux 配置，未启动 Codex daemon 或结构化适配服务。构建、使用说明与具体限制统一维护在子项目 README，后续先完成真机验证，再考虑聊天式增强。
