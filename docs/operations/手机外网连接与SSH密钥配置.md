...
| 公网主机 / 转发端口 | 用户已确认尚未配置 SSH 公网域名及外部端口；需新建入口并验收，不能沿用 Web 入口作为 SSH 地址 |
...
当前状态：用户确认尚未配置。先完成内网新密钥与 App 验证，再设置独立 SSH 转发；可先用路由器公网 IPv4 测试，DDNS 随后配置。外部 `22222/TCP → 192.168.50.33:22` 仅为待核对空闲端口的示例，尚未部署。
