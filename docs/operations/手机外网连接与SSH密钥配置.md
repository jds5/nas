...
更新：2026-09-17。本仓库所在的 `ynas` 就是维护目标和 App 连接目标，当前对话运行在该机 `yao` 用户的 tmux 中。App 主要在外部 Wi-Fi / 蜂窝网络使用；局域网下载页仅用于分发测试 APK。
...
| 公网主机 / 转发端口 | 用户提供 `jelly.rokano.org:2257`、用户 `yao`；定位 UFW 仅允许 LAN SSH 并提供放行步骤后，2026-09-17 用户确认 4G 连接恢复 |
...
2026-09-17 用户随后反馈连接恢复。以下保留本次修复与回滚步骤供维护参考；助手没有管理员权限，未代执行或独立读取修改后规则，已恢复时无需重复添加。需要实施时保留现有管理连接，先成功备份：
