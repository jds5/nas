...
| 公网主机 / 转发端口 | 2026-09-16 用户反馈已成功通过外网连接 App；实际使用的域名/IP、端口尚未提供，不将示例值当成现场配置 |
...
当前状态：2026-09-16 用户确认，授予局域网权限后 App 内网连接成功，随后外网连接也成功。以下保留配置方法供维护参考；`22222/TCP → 192.168.50.33:22` 是示例，实际域名/IP 与外部端口尚未提供。断线重连、Wi-Fi/蜂窝切换及公网 IP 变化后的 DDNS 恢复仍待验证。
...
同家庭局域网手机打开 [下载页](http://192.168.50.33:8765/)，获取 0.1.1 安装包。不要把 8765 转发公网，也不要在页面上传私钥。下载服务管理见 [分发说明](../../apps/android/distribution/README.md)。
