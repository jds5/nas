...
- 管理面默认仅 LAN；需要远程维护时采用自建 WireGuard → SSH。Tailscale 退出是目标，先核对 DNS、路由与客户端依赖，并验收替代路径后再停用。
...
此前关于公网 SSH、Android 动态白名单和自研客户端的讨论没有形成部署决定，本次移除评估全文与工时估算。当前管理入口仍按 LAN / VPN 方案维护；应用远控功能需要另行测试，不能把恢复历史会话当成接管正在运行的终端。
