...
- 表格直接在聊天正文中完整显示，不弹出对话框，也不按七条数据行折叠；横向滑动、位置条和左右按钮保留。包含表格的消息跳过通用的字符截断预览，避免表格被切在半行；既有每块 12 列/200 行与单条消息上限仍保留。普通长文字仍可在原位置展开。
- 输入入口调整为“截图 / 相册”“拍照”“文件…”。截图/相册调用系统 Photo Picker，按最近图片浏览，不默认打开文件目录；系统支持时可选截图相册。不是录屏或跨 App 截图功能，不申请全相册/存储权限。旧设备没有 Photo Picker 时由 AndroidX 回退到文件选择器。
- 拍照调用系统相机，通过仅开放 `cache/camera/` 的非导出 FileProvider 临时授予单张照片访问权。照片复制为待发送附件后或取消拍照时清理来源并撤销授权；不会自动上传或保存到公共相册。系统缺少相机/选择器时显示提示。
- 文件/相册/相机结果回调在应用顶层注册，不再随聊天页退出而注销。选择目标在打开选择器前记录，与 SSH 连接代次分离；选取期间断线或超过两分钟，结果仍可成为待发送附件。重新接回相同主机、用户、指纹、窗格和 Codex 后发送；不会转交其他执行器，也不会自动重发。
- 两分钟只限制内存中已解锁 SSH 身份的保留，不再使附件选择作废。超时后仍需重新输入私钥口令；已选附件和正在选择的结果保留于当前 App 进程。连接页会提示待发送附件数量，接回原会话不清空附件。切换目标/清除资料/发送成功才按规则清理。

此版没有新增后台保活或附件持久化：如果系统杀死 App 进程、主动退出后重新启动或系统回收缓存，附件仍可能需要重选。系统相机、超过两分钟选图、选择器返回时换网及完整表格滚动需真机回归。43 项 Python、22 项 JVM、3 项下载测试，以及 Debug/Release/Lint 通过；本轮新增 4 项附件目标恢复/隔离测试不替代 Android 生命周期实机测试。

实现依据：[Android Photo Picker](https://developer.android.com/training/data-storage/shared/photo-picker)、[TakePicture](https://developer.android.com/reference/androidx/activity/result/contract/ActivityResultContracts.TakePicture)、[FileProvider](https://developer.android.com/reference/androidx/core/content/FileProvider)。0.4.0 下方记录的弹框和 8 行预览已由本版替代。
