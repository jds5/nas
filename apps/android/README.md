...
独立 Android 子项目：通过 SSH 接续 NAS 上正在运行的 Codex。当前为 0.3.1，默认展示聊天消息，普通输入框一次发送；原终端模式保留为可选入口。无需新端口或 NAS 常驻服务，不新建、恢复或分叉正在控制的 Codex。
...
6. 离开 App 或锁屏会关闭 SSH；远端任务继续。返回后可点“接回上次会话”，有私钥口令时仍需输入；原执行器变化则停止。电脑和手机共享输入，请避免同时操作。
...
- 不持久化对话输出、私钥口令或连接日志。最多 8 个会话的执行器身份、阅读位置和发送结果标记加密保存；草稿默认仅在内存，可在连接页开启“加密保存草稿”，包含普通输入和自由回答草稿。使用独立 Keystore 密钥、AES-GCM 与原子文件，文件上限 2 MiB；关闭选项后删除磁盘记录中的草稿，清除本机资料会同时删除这些记录。普通编辑以 300 ms 防抖保存，突然杀进程可能丢失最后一次编辑。
...
- 输出使用原生文字组件，可选择复制，支持标题、列表、粗体、行内代码、代码块和简单表格（每块最多 12 列/200 行，超限部分原文显示）；不执行 ANSI/OSC、HTML、链接或远端剪贴板动作，不加载远程图片。SSH 单次响应上限 256 KiB；聊天每次扫描至多 1 MiB，返回至多 80 条/约 180 KB 消息，单条上限 24,000 字符，App 内至多 300 条；终端显示约 400 行。
...
- 只转换当前 CLI 能完整识别的异步问题布局；截断、已有电脑端自由回答、未知布局、超过 8 KiB 的题目/选项数据使用控制面板。权限审批不套用此表单。当前自定义回答支持单行、最多 4 KiB，复杂输入保留手动入口。
