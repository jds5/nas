...
python3 apps/android/distribution/serve.py \
  ...
  --apk apps/android/distribution/build/nas-remote-0.2.0.apk --version 0.2.0
```
...
apksigner sign --ks "$HOME/.android/debug.keystore" --ks-key-alias androiddebugkey \
  ...
  --out apps/android/distribution/build/nas-remote-0.2.0.apk \
  apps/android/app/build/outputs/apk/release/app-release-unsigned.apk
apksigner verify --verbose apps/android/distribution/build/nas-remote-0.2.0.apk
```
